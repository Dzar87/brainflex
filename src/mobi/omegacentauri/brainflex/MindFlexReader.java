...
public class MindFlexReader {
    ...
    public MindFlexReader(BrainFlexGUI gui, DataLink dataLink, int mode, File saveFile) {
        ...
        lastSignal = 100;
        badPacketCount = 0;
        this.soundT = new SoundThread("SOUNDTHREAD");
    }
...
//        }
        ...
        switch(code) {
            ...
            case (byte)0x04:
                ...
                gui.log("ATTENTION "+v);
                soundT.setMedAttValue(v, AttentionType.ATTENTION);
                // SEND TO SOUND THREAD 0-100
            ...
            case (byte)0x05:
                ...
                // SEND TO SOUND THREAD 0-100
                soundT.setMedAttValue(v, AttentionType.MEDITATION);
...
//	}
    ...
    public void stop() {
        // Terminate the soundThread before everything else.
        this.soundT.end();
        try {
            // Join the soundT to make sure it actually terminates before main
            this.soundT.join();
        } catch (InterruptedException e) {}
